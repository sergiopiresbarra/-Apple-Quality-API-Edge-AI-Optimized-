...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # "*" significa: Libera para TODO MUNDO (Celular, PC, etc)
...
def ler_historico():
    conn = sqlite3.connect('historico.db')
    # Isso faz o banco retornar dicionários (chave: valor) em vez de números soltos
    conn.row_factory = sqlite3.Row 
    cursor = conn.cursor()
    
    # Pega os últimos 20 registros (do mais novo para o mais antigo)
    cursor.execute("SELECT * FROM auditoria ORDER BY id DESC LIMIT 20")
    
    # Converte para uma lista que o JSON entende
